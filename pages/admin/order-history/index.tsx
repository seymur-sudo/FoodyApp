...
const OrderHistory: React.FC = () => {
  const { closeUserModal, modalSpring, openUserModal, showUserModal } =
    useSidebarContext() as SidebarContextProps;

  const { t } = useTranslation("common");
  const [sortingValue, setSortingValue] = useState<SortingValue>("A-Z");
  const [historytems, setHistoryItems] = useState<orderItem[]>();
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery(QUERIES.Order, getHistory);
  const orderList = history?.data.result.data;
      ...
      <div className="bg-bgc h-screen px-12 md:px-6">
        ...
        <div className=" overflow-x-auto shadow-md sm:rounded-lg">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {sortedProducts &&
                sortedProducts.map((history: OrderPostDataType, index) => (
                  <tr
                    key={history.id}
                  ...
                  >
                    ...
                    <td className="pr-8 py-4 font-semibold text-gray-900 dark:text-white capitalize">
                      {Moment(history.created).format("DD-MM-YY - HH:mm:ss")}
                    </td>
                    ...
                    <td className="pl-5 py-4">
                      <div className="flex items-center">
                        <FaEye
                          onClick={() => handleOrderShow(history.products)}
                          className="hover:scale-110 transition-all duration-500 ml-3 text-cyan-400  text-[27px] cursor-pointer"
                        />
                      </div>
                    </td>
                  </tr>
